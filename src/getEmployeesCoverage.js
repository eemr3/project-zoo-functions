...
const getEmployeesCoverageNotParams = () => employees.map((employee) => ({
  id: employee.id,
  fullName: `${employee.firstName} ${employee.lastName}`,
  species: species.reduce((acc, specie) => (employee.responsibleFor.includes(specie.id)
    ? acc.concat(specie.name) : acc), []),
  locations: species.reduce((accLocation, location) => (
    employee.responsibleFor.includes(location.id)
      ? accLocation.concat(location.location) : accLocation), []),
}));

const isEmployeesCoverage = ({ id, name }) => employees.some((employee) =>
  employee.id === id || employee.firstName === name || employee.lastName === name);
